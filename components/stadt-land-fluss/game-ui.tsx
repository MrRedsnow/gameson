...
export function SlfTimerDialog({ state, busy, send, onClose }: { state: SlfState; busy: boolean; send: SlfSend; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="slf-theme slf-dialog"><DialogTitle>Timer bearbeiten</DialogTitle><DialogDescription>{writing ? "Die neue Restzeit gilt ab dem Speichern für alle. Sie wird auch für die nächsten Runden übernommen." : "Lege die Zeit für die nächste Runde fest. Ohne Timer endet die Schreibphase, wenn alle abgeben oder du sie beendest."}</DialogDescription>
