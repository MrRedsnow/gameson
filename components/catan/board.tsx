...
export function CatanBoard({ game, mode, choices, selected, onSelect, disabled }: {
  ...
  const viewBox = focus ? `${focus.x - cameraWidth / 2} ${focus.y - cameraHeight / 2} ${cameraWidth} ${cameraHeight}` : "-306 -278 612 556";
  ...
  return <section className="catan-board-panel" aria-label="Catan-Spielbrett">
    <div ref={viewport} className="catan-board-viewport">
      <svg className="catan-board" viewBox={viewBox} role="group" aria-label="Insel mit Landschaften, Häfen, Straßen und Siedlungen" data-focused={Boolean(focus)}>
        ...
        {(mode === "settlement" || mode === "city") && choices.map((id) => {
          ...
          return <g key={id} className="catan-map-target" {...interactive(id, `${mode === "city" ? "Stadt" : "Siedlung"} auf Kreuzung ${id + 1} bauen`)}>
            <circle cx={v.x} cy={v.y} r="22" fill="transparent" /><circle cx={v.x} cy={v.y} r="18" fill={selected === id ? "#ffe7a1" : "#162b34"} stroke="#ffe7a1" strokeWidth="3" />
    ...
    <div className="catan-board-controls">
      {mode ? <>
        ...
        <Button variant="outline" className="catan-map-overview" disabled={disabled || !choices.length} onClick={() => { if (focus) setOverview(selectionKey); else activate(selected ?? choices[0]); }}>{focus ? <MapIcon /> : <ZoomIn />}<span>{focus ? "Ganze Insel" : "Bauplätze ansehen"}<small>{index >= 0 ? `Platz ${index + 1} / ${choices.length}` : `${choices.length} mögliche Plätze`}</small></span></Button>
        ...
        <Button variant="outline" className="catan-map-overview" onClick={() => setInspect(inspect === null ? 9 : null)}>{inspect !== null ? <MapIcon /> : <ZoomIn />}<span>{inspect !== null ? "Ganze Insel" : "Felder ansehen"}<small>{inspect !== null ? `${board.hexes[inspect].resource === "desert" ? "Wüste" : RESOURCE_INFO[board.hexes[inspect].resource as Resource].terrain} · ${board.hexes[inspect].number ?? "kein Ertrag"}` : "Feld antippen zum Vergrößern"}</small></span></Button>
