...
import { BagText, ResourceAmounts, ResourceChoices, Screen, type Send } from "./play-primitives";
...
export function TradePanel({ game, send, busy, turnActions }: { game: CatanView; send: Send; busy: boolean; turnActions?: ReactNode }) {
  ...
  if (offer && view === "choice") {
    ...
    return <Screen title={incoming ? `Angebot von ${from.name}` : ownOffer ? `Angebot an ${to.name}` : `${from.name} handelt mit ${to.name}`} eyebrow="Offener Handel" actions={<>
      ...
      <div className="catan-exchange"><p><span>{involved ? "Du gibst" : `${from.name} gibt`}</span><strong><BagText bag={payment} /></strong></p><p><span>{involved ? "Du erhältst" : `${from.name} erhält`}</span><strong><BagText bag={receipt} /></strong></p></div>
      {involved && <TradeInventoryPreview resources={me.resources} give={payment} receive={receipt} />}
      {!incoming && <p className="catan-inline-hint">{ownOffer ? `${to.name} entscheidet über dein Angebot.` : "Dieser Handel muss zuerst abgeschlossen werden."}</p>}
    </Screen>;
  }
  ...
  if (view === "review") return <Screen title={`Dein Angebot an ${partner!.name}`} eyebrow="Schritt 4 von 4" back={() => setView("receive")} actions={<Button className="catan-primary" disabled={busy || !resourceCount(offered) || !resourceCount(wanted) || !canAfford(me.resources, offered) || Boolean(mixed) || Boolean(offer && ![offer.fromId, offer.toId].includes(me.id))} onClick={async () => { if (await send({ type: "offer_trade", toId: partner!.id, give: offered, receive: wanted })) reset(); }}>{offer?.toId === me.id ? "Gegenangebot senden" : "Handel anbieten"}</Button>}>
    <div className="catan-exchange"><p><span>Du gibst</span><strong><BagText bag={offered} /></strong></p><p><span>Du erhältst</span><strong><BagText bag={wanted} /></strong></p></div>
    <TradeInventoryPreview resources={me.resources} give={offered} receive={wanted} unavailable={mixed} />
  </Screen>;
