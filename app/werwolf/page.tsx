...
function WaitingRoom({ state, busy, post, invite, settings, audioReady, enableAudio }: { state: LobbyState; busy: boolean; post: PostAction; invite: () => void; settings: () => void; audioReady: boolean; enableAudio: () => Promise<void> }) {
  ...
  return <section className="lobby-workspace">
    ...
    <Card className={`lobby-audio-card ${audioReady ? "ready" : ""}`}><CardContent><div className="audio-card-heading"><AudioLines aria-hidden="true" /><strong>Ansagen vorlesen lassen <Badge variant="outline">Optional</Badge></strong></div><p>{playsHere ? audioReady ? "Der Ton ist auf diesem Gerät eingeschaltet." : "Aktiviere den Ton, wenn du die Spielansagen hören möchtest. Du kannst auch alle Schritte auf dem Bildschirm lesen." : `Die Ansagen laufen bei ${host}. Auf deinem Gerät ist nichts einzustellen.`}</p>{playsHere && <Button variant="outline" disabled={audioReady} onClick={() => void enableAudio()}>{audioReady ? <Check aria-hidden="true" /> : <AudioLines aria-hidden="true" />}{audioReady ? "Ton ist eingeschaltet" : "Ton einschalten"}</Button>}</CardContent></Card>
...
function LocalWerewolf({ onBack, showError }: { onBack: () => void; showError: (error: unknown) => void }) {
  ...
  const start = () => { if (validNames.length < 3) return showError(new Error("Füge mindestens drei Namen hinzu.")); if (new Set(validNames.map((name) => name.toLocaleLowerCase("de"))).size !== validNames.length) return showError(new Error("Jeder Name darf nur einmal vorkommen.")); const error = validateRoleSetup(validNames.length, effectiveWolves, roles); if (error) return showError(new Error(error)); void unlockWerewolfAudio().then(() => setAudioReady(true)).catch(showError); localCue.current = ""; localWinnerCue.current = ""; localAnnouncedNight.current = 0; localInitialSleep.current = false; const deck = buildRoleDeck(validNames.length, effectiveWolves, roles, randomIndex); const next = validNames.map((name, index) => { const role = deck[index]; return { id: crypto.randomUUID(), name, role, team: roleTeam(role), alive: true, loverId: null, roleModelId: null, charmed: false, elderShield: role === "elder", healPotion: role === "witch", poisonPotion: role === "witch", lastProtectedId: null, deathCauses: [] }; }); setReceipt(""); setReserveRoles(["thief", "villager", randomIndex(2) ? "werewolf" : "villager"]); setPlayers(next); setMayorId(null); setNight(0); setWinner(null); setVoteHistory([]); setRevealIndex(0); setReady(false); setPhase("reveal"); };
