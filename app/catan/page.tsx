...
function loadLocal(): { game: CatanGame; seen: Record<string, number> } | null {
  const raw = localStorage.getItem(LOCAL_KEY); if (!raw) return null;
  const { localReceipts = [], localSeen, ...value } = JSON.parse(raw) as CatanGame & { localSeen?: Record<string, number>; localReceipts?: { playerId: string; afterSequence: number }[] };
  ...
  return { game: value, seen: restoreLocalSeen(value, localSeen, localReceipts) };
}
...
export default function CatanPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (startup.kind === "local") { const saved = loadLocal(); if (saved) { setLocalGame(saved.game); setLocalSeen(saved.seen); } else setMode("local"); }
  ...
  function startLocal() {
    try {
      ...
      const seen = Object.fromEntries(game.players.map((p) => [p.id, 0]));
      store(LOCAL_KEY, JSON.stringify({ ...game, localSeen: seen })); setLocalGame(game); setLocalSeen(seen); setLocalSaved(true); setUnlocked(null); setNotice("");
  ...
  return <main className={`catan-shell ${playing ? "catan-shell-wide catan-shell-play" : needsHandoff ? "catan-shell-handoff" : ""}`}>
    ...
    {!ready ? <p role="status">Spiel wird geladen …</p> : session && !state ? <section className="catan-panel"><h1>Lobby wird geladen …</h1><p role="status">{connected ? "Deine letzte Partie wird fortgesetzt." : "Die Verbindung fehlt. Wir versuchen es erneut."}</p><Button variant="outline" onClick={() => { setSession(null); back(); }}>Zur Spielauswahl von Catan</Button></section> : needsHandoff ? <section className="catan-handoff">
      ...
      <section className={`catan-intro ${mode !== "home" ? "is-compact" : ""}`}><span className="catan-kicker">Handel · Strategie · Inselglück</span><h1>Die Siedler{mode === "home" ? <br /> : " "}von <span>Catan.</span></h1><p>Straßen verbinden. Siedlungen wachsen.<br />Wer erreicht zuerst das Punktziel?</p><div className="catan-intro-meta"><span>3–4 Personen</span><span>60–120 Minuten</span><span>Ab 10 Jahren</span></div></section>
      {mode === "home" ? <>
        <GameModes onCreate={() => setMode("create")} onJoin={() => setMode("join")} onLocal={() => setMode("local")} />
        {localSaved && <Button className="catan-resume" variant="outline" onClick={() => { try { const saved = loadLocal(); if (saved) { setLocalGame(saved.game); setLocalSeen(saved.seen); setUnlocked(null); window.history.replaceState({}, "", "/catan?local=1"); } else setLocalSaved(false); } catch (error) { setNotice((error as Error).message); } }}><RotateCcw />Lokale Partie fortsetzen</Button>}
