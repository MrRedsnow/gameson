...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const hydrate = window.setTimeout(() => {
      ...
      else { try { const stored = localStorage.getItem("imposter-session"); if (stored) setSession(JSON.parse(stored) as Session); } catch { /* empty */ } }
    }, 0);
  ...
  useEffect(() => {
    ...
    window.history.replaceState({}, "", `?lobby=${encodeURIComponent(session.lobbyId)}`);
  }, [session]);
  ...
  const fetchState = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("Sitzung")) { localStorage.removeItem("imposter-session"); setSession(null); setState(null); window.history.replaceState({}, "", "/"); }
    }
  ...
  const leaveToHome = () => {
    setSession(null); setState(null); setAssignment(null); localStorage.removeItem("imposter-session"); window.history.replaceState({}, "", "/"); setScreen("home");
  };
  ...
  if (screen === "join") return <JoinLobby nearby={nearby} inviteLobbyId={inviteLobbyId} onSelectLobby={setInviteLobbyId} onBack={() => { setInviteLobbyId(""); setScreen("home"); window.history.replaceState({}, "", "/"); }} onJoined={setSession} showError={showError} />;
...
function MultiGame({ state, assignment, session, online, busy, post, onBack, showError }: { state: LobbyState | null; assignment: Assignment | null; session: Session; online: boolean; busy: boolean; post: (action: string, values?: Record<string, unknown>) => Promise<void>; onBack: () => void; showError: (error: unknown) => void }) {
  ...
  const shareUrl = typeof window === "undefined" ? "" : `${window.location.origin}/?lobby=${session.lobbyId}`;
  ...
  return (
    <main className="app-shell game-shell">
      ...
      {state.lobby.status === "results" && <Results state={state} busy={busy} post={post} />}
