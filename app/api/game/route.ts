...
type LobbyRow = {
  ...
  discoverable: number; revision: number; network_hash: string; round_number: number; created_at: number; updated_at: number;
};
...
async function cleanup() {
  const db = getD1();
  const cutoff = Date.now() - 12 * HOUR;
  await db.batch([
    db.prepare("DELETE FROM assignments WHERE round_id IN (SELECT id FROM rounds WHERE lobby_id IN (SELECT id FROM lobbies WHERE updated_at < ?))").bind(cutoff),
    db.prepare("DELETE FROM votes WHERE round_id IN (SELECT id FROM rounds WHERE lobby_id IN (SELECT id FROM lobbies WHERE updated_at < ?))").bind(cutoff),
    db.prepare("DELETE FROM rounds WHERE lobby_id IN (SELECT id FROM lobbies WHERE updated_at < ?)").bind(cutoff),
    db.prepare("DELETE FROM custom_pairs WHERE lobby_id IN (SELECT id FROM lobbies WHERE updated_at < ?)").bind(cutoff),
    db.prepare("DELETE FROM players WHERE lobby_id IN (SELECT id FROM lobbies WHERE updated_at < ?)").bind(cutoff),
    db.prepare("DELETE FROM lobbies WHERE updated_at < ?").bind(cutoff),
...
export async function GET(request: Request) {
  try {
    ...
    if (now - player.last_seen > 12000) {
      ...
      await db.batch([
        db.prepare("UPDATE players SET last_seen = ? WHERE id = ?").bind(now, player.id),
        db.prepare("UPDATE lobbies SET updated_at = ?, network_hash = CASE WHEN host_player_id = ? THEN ? ELSE network_hash END WHERE id = ?").bind(now, player.id, nextNetwork, lobby.id),
      ]);
    }
...
async function createRound(lobby: LobbyRow, players: PlayerRow[]) {
  ...
  const statements = [
    ...
    db.prepare("UPDATE lobbies SET status = 'revealing', round_number = ?, revision = revision + 1, updated_at = ? WHERE id = ? AND status IN ('waiting', 'results')").bind(roundNumber, now, lobby.id),
  ];
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "create") {
      ...
      if (await db.prepare("SELECT id FROM lobbies WHERE normalized_name = ?").bind(normalized).first()) return fail("Dieser Gruppenname ist gerade schon vergeben.", 409);
    ...
    if (action === "join") {
      ...
      if (await db.prepare("SELECT id FROM players WHERE lobby_id = ? AND normalized_name = ?").bind(lobby.id, normalizeName(playerName)).first()) return fail("Dieser Spielername ist in der Lobby schon vergeben.", 409);
    ...
    if (action === "open_vote" || action === "finish_vote") {
      ...
      await db.batch([
        ...
        db.prepare("UPDATE lobbies SET status = ?, revision = revision + 1, updated_at = ? WHERE id = ? AND status = ?").bind(next, Date.now(), lobbyId, expected),
      ]);
      return reply({ ok: true });
    }
