...
function Topbar({ title, onBack, online, local = false }: { title?: string; onBack?: () => void; online?: boolean; local?: boolean }) {
  return (
    <header className="topbar">
      ...
      <span className={`connection-pill ${local ? "local" : online === false ? "offline" : ""}`} aria-label={local ? "Lokales Spiel – kein Internet nötig" : online === false ? "Offline" : "Online"}><i />{local ? "lokal" : online === false ? "offline" : "online"}</span>
    </header>
  );
}
...
function CreateLobby({ onBack, onCreated, showError }: { onBack: () => void; onCreated: (session: Session) => void; showError: (error: unknown) => void }) {
  ...
  return <main className="app-shell"><Topbar title="Neue Lobby" onBack={onBack} /><section className="page-intro"><span className="step-label">01 · Gruppe</span><h2>Gebt eurer Runde einen Namen.</h2><p>Danach kannst du Spieler einladen und die Wörter einstellen.</p></section><form className="form-card" onSubmit={submit}><label>Gruppenname<input value={groupName} onChange={(e) => setGroupName(e.target.value)} placeholder="z. B. Wohnzimmer" minLength={2} maxLength={28} autoComplete="off" required /></label><label>Dein Name<input value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder="z. B. Mia" minLength={2} maxLength={24} autoComplete="nickname" required /></label><button className="primary-button" disabled={busy}>{busy ? "Wird erstellt …" : "Lobby erstellen →"}</button><p className="privacy-note">Keine Konten. Beendete Lobbys werden nach 30 Minuten, inaktive nach 12 Stunden gelöscht.</p></form></main>;
}
...
function JoinLobby({ nearby, inviteLobbyId, onSelectLobby, onBack, onJoined, showError }: { nearby: NearbyLobby[]; inviteLobbyId: string; onSelectLobby: (id: string) => void; onBack: () => void; onJoined: (session: Session) => void; showError: (error: unknown) => void }) {
  ...
  return <main className="app-shell"><Topbar title="Beitreten" onBack={onBack} /><section className="page-intro"><span className="step-label">01 · Beitreten</span><h2>{selectedLobby ? `„${selectedLobby.name}“ wartet auf dich.` : inviteLobbyId ? "Einladung erhalten." : "Finde deine Gruppe."}</h2><p>{inviteLobbyId ? "Nur noch deinen Namen eingeben – dann bist du dabei." : "Wähle eine nahe Lobby oder gib ihren Gruppennamen ein."}</p></section>{selectedLobby && <div className="selected-lobby"><span><small>Ausgewählte Lobby</small><strong>{selectedLobby.name}</strong></span><button type="button" onClick={() => onSelectLobby("")}>Ändern</button></div>}{!inviteLobbyId && nearby.length > 0 && <section className="nearby-section"><div className="section-heading"><strong>In deiner Nähe</strong><span>automatisch erkannt</span></div><div className="nearby-list">{nearby.map((item) => <button key={item.id} type="button" onClick={() => onSelectLobby(item.id)}><span className="nearby-pulse" /><span><strong>{item.name}</strong><small>{item.player_count} {item.player_count === 1 ? "Person" : "Personen"}</small></span><b>→</b></button>)}</div></section>}<form className="form-card" onSubmit={submit}>{!inviteLobbyId && <label>Gruppenname<input value={groupName} onChange={(e) => setGroupName(e.target.value)} placeholder="Gruppenname" minLength={2} maxLength={28} autoComplete="off" required /></label>}<label>Dein Name<input value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder="Dein Spielername" minLength={2} maxLength={24} autoComplete="nickname" required /></label><button className="primary-button" disabled={busy}>{busy ? "Wird verbunden …" : "Lobby beitreten →"}</button></form></main>;
}
...
function LocalGame({ onBack, showError }: { onBack: () => void; showError: (error: unknown) => void }) {
  ...
  return <main className="app-shell"><Topbar title="Ergebnis" local /><section className="results-screen"><div className="result-stamp">Abgestimmt</div><div className="page-intro"><span className="step-label">Ergebnis</span><h2>Das sagt die Gruppe.</h2><p>Die Rollen bleiben geheim.</p></div><div className="result-list">{voteTotals.map((item, idx) => <div className={idx === 0 && item.votes > 0 ? "leader" : ""} key={item.id}><span><strong>{item.name}</strong><b>{item.votes}</b></span><i><em style={{ width: `${(item.votes / Math.max(1, voteTotals[0]?.votes ?? 1)) * 100}%` }} /></i></div>)}</div><button className="primary-button coral" onClick={start}>Neue Runde →</button><button className="secondary-button" onClick={() => setPhase("setup")}>Spieler ändern</button></section></main>;
}
